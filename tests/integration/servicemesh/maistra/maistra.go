...
func ApplyGatewayAPICRDs(ctx resource.Context) error {
	for _, c := range ctx.Clusters() {
		if err := c.ApplyYAMLFiles("", filepath.Join(env.IstioSrc, "tests/integration/servicemesh/maistra/testdata/gateway-api-crd.yaml")); err != nil {
			return err
		}
	}
	return nil
...
func Install(opts InstallationOptions) resource.SetupFn {
	return func(ctx resource.Context) error {
		...
		if err := ctx.Clusters().Default().ApplyYAMLFiles("", filepath.Join(env.IstioSrc, "tests/integration/servicemesh/maistra/testdata/clusterrole.yaml")); err != nil {
			return err
		}
